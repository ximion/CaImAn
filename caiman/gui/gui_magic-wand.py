# -*- coding: utf-8 -*-
"""
Created on Thu Oct 20 11:13:12 2016
...
imp.load_source('cmw','../CellMagicWand/cell_magic_wand.py')
import cmw
...
class GUI_magic_wand(animation.TimedAnimation):
    """
    ...
    def __init__(self, mov, roi, traces, images={}, cmap=pl.cm.viridis, **kwargs):
        ...
        self.traces = traces
        self.images = images
        self.mask = np.ones(self.images['mean'].shape)
        ...
        if len(self.images):
            ...
            self.imgdata = self.ax_img.imshow(im,cmap='gray')
            #self.imgdata = self.ax_img.imshow(self.mask,cmap='gray')
    ...
    def evt_magic_wand(self,evt):
        print(evt.x)
        print(evt.y)
        
        x,y = int(np.round(evt.xdata)), int(np.round(evt.ydata))
        mask = cmw.cell_magic_wand_single_point(self.images['mean'],(y,x),1,10,roughness = 2)[0]
        print(mask)       
        # self.imgdata.set_data(self.images[0]*mask)
...
#        import time
    ...
    def set_current_trace(self, idx):
        # temporarily removing until integrated better with ROIs
        #col = self.cmap(np.linspace(0,1,np.sum(self.roi_kept)))[np.squeeze(idx)]
        
        col = self.cmap(np.linspace(0,1,))[np.squeeze(idx)]
        t = self.traces[:,idx]
    ...
    def set_specific_trace(self, idx):
        # temporarily removing until integrated better with ROIs
        #col = self.cmap(np.linspace(0,1,np.sum(self.roi_kept)))[np.squeeze(idx)]
...
if __name__ == '__main__':
    ...
    mov = load('example_movies/demoMovie.tif',fr=15)
    mean,minn,maxx = mov.mean(axis=0),mov.min(axis=0),mov.max(axis=0)
    masks=np.zeros([3,60,80])
    masks[0,20:50,50:70]=1
    masks[1,2:10,5:10]=1
